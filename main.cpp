...
static int open_input_file(uint8_t *array, const size_t size, const char *src_format_short_name)
{
  int ret;
  unsigned int i;
  ...
  const AVInputFormat *input_format = throw_if_null(av_find_input_format(src_format_short_name), "av_find_input_format failed");
  printf("opened\n");
  if ((ret = avformat_open_input(&ifmt_ctx, NULL, input_format, NULL)) < 0)
  {
  ...
  av_dump_format(ifmt_ctx, 0, "<input_buffer>", 0);
  return 0;
}

static int open_output_file(const char *dst_format_short_name, const char *dst_codec_name)
{
  AVStream *out_stream;
  AVStream *in_stream;
  ...
  throw_if_neg(avformat_alloc_output_context2(&ofmt_ctx, output_format, nullptr, nullptr), "Failed to allocate output context");
  ...
  for (i = 0; i < ifmt_ctx->nb_streams; i++)
  {
    ...
    if (dec_ctx->codec_type == AVMEDIA_TYPE_VIDEO || dec_ctx->codec_type == AVMEDIA_TYPE_AUDIO)
    {
      ...
      encoder = throw_if_null(avcodec_find_encoder_by_name(dst_codec_name), "Destination codec not found");
      if (!encoder)
      {
  ...
  av_dump_format(ofmt_ctx, 0, "<output_buffer>", 1);
  ...
  throw_if_neg(avformat_write_header(ofmt_ctx, nullptr), "Failed to write header");
...
static int encode_write_frame(unsigned int stream_index, int flush)
{
  ...
  filt_frame->nb_samples = stream->enc_ctx->frame_size;
  ret = throw_if_neg(avcodec_send_frame(stream->enc_ctx, filt_frame), "avcodec_send_frame failed");
  ...
  while (ret >= 0)
  {
    ...
    throw_if_neg(ret, "avcodec_receive_packet error");
    ...
    /* mux encoded frame */
    throw_if_neg(av_interleaved_write_frame(ofmt_ctx, enc_pkt), "av_interleaved_write_frame error");
  }
...
extern "C"
{
  ...
  int transcode(uint8_t *array, const size_t size, const char *src_format_short_name, const char *dst_format_short_name, const char *dst_codec_name)
  {
    ...
    if ((ret = open_input_file(array, size, src_format_short_name)) < 0)
      goto end;
    if ((ret = open_output_file(dst_format_short_name, dst_codec_name)) < 0)
      goto end;
    ...
    printf("transcoded ended, size: %zu %d\n", result.size(), result[1000]);
    ...
    throw_if_neg(ret, "Error occurred");
